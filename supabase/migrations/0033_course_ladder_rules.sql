-- The ladder rule has been rewritten whole five times — 0019, 0023, 0024, 0029,
-- 0032 — because it had no home. Two separate questions were spelled out inside
-- student_leaderboard's CTEs: who may read a course's ladder, and who appears on
-- it. Neither could be asked on its own, so every correction meant restating a
-- 180-line report query and reading the result in production.
--
-- This gives each question a function. student_leaderboard keeps the reporting
-- (xp, mastery, streak, ranking) and asks for the rules instead of restating
-- them. Behaviour is unchanged from 0032.
--
-- Both predicates stay internal: revoked from public, granted to nobody. A
-- security definer function's callees are checked against the definer, so
-- student_leaderboard can call them while a client cannot enumerate the
-- participants of a course it has no business reading.

-- May this user read this course's ladder at all?
-- A learner sees the ladder of a published or archived course they are enrolled
-- in as a student. A community author sees their own course's ladder without
-- being enrolled in it.
create or replace function public.can_view_course_ladder(
  p_user_id uuid,
  p_course_id uuid
)
returns boolean
  ...
  select exists (
    ...
    where c.id = p_course_id
      ...
      and (
        ...
        or (c.course_kind = 'community' and c.owner_id = p_user_id)
      )
  );
$$;

revoke all on function public.can_view_course_ladder(uuid, uuid) from public;

-- Who appears on this course's ladder.
-- Enrolled students who opted in to peer visibility, plus the caller themselves
-- so a private student still sees their own standing — unless they are a
-- verified instructor, who stays out of the learner ladder entirely. A
-- community author joins their own learners' ladder only by the same opt-in,
-- and the union covers older published courses whose owner enrollment was
...
revoke all on function public.course_ladder_participants(uuid) from public;

-- The report. Scope and participation are now asked for, not restated.
...
as $$
  with scope_course as (
    ...
    where p_course_id is not null
      ...
      and public.can_view_course_ladder(auth.uid(), c.id)
  ),
  candidates as (
    select participants.user_id
    from scope_course sc
    cross join lateral public.course_ladder_participants(sc.id) as participants
  ),
  course_nodes as (
