-- A verified instructor is not a learner, and ranking against the class is a
-- thing they opt into rather than out of.
--
-- The course owner was already excluded twice. The gap was an instructor
-- enrolled as a student in someone else's course: `social_opt_in` kept them off
-- everyone else's board, but the clause that always returns the caller's own
-- row put them back on their own. That escape hatch now stops at a verified
-- instructor, so `social_opt_in` is the single switch for them — off by default,
-- because a profile starts with it false — and the rule holds however the
-- leaderboard RPC is called.
--
-- Owner exclusion stays absolute: opting in ranks an instructor in other
-- people's courses, never in their own. Student semantics are untouched — an
-- opted-out student still sees their own rank.
