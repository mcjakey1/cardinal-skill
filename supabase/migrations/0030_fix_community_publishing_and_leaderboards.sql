-- Community publishing is catalog-based. It does not create a second invite
-- path or expose an opaque code that learners would have to carry around.
drop function public.publish_community_course(uuid, text);

create function public.publish_community_course(
  p_course_id uuid,
...
-- Community authors may inspect their course ladder without becoming a
-- participant. Learners still need a student enrollment, and the author is
-- excluded from candidates even if an old enrollment row exists.
